...
function save_supplier(){
    ...
    $data .= ", price = '$price' "; // Add the price field

    // Check for duplicate entry
    $chk = $this->db->query("SELECT * FROM suppliers WHERE name = '$name' AND id != '$id'")->num_rows;
    if($chk > 0){
        return 2;
        exit;
    }
...
function delete_supplier(){
    ...
    $check = $this->db->query("SELECT * FROM batch_lot where item_name ='$item_name' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
    if($check > 0){
        return 2; // Item name already exists
    }
    if(empty($id)){
        $save = $this->db->query("INSERT INTO batch_lot SET $data");
    } else {
        $save = $this->db->query("UPDATE batch_lot SET $data WHERE id = $id");
    }
    if($save){
        return 1; // Success
    }
...
function delete_batch_lot(){
    extract($_POST);
    $delete = $this->db->query("DELETE FROM batch_lot where id = ".$id);
    if($delete){
        return 1; // Success
    }
}
